...
| 只读令牌 | Bot/脚本用：`dshk_` + 256 位随机量，服务端只存 SHA-256；**只能 GET**，写接口一律 403；令牌不能管理令牌；撤销即时生效 |
...
给 Bot 发**只读令牌**而不是账号密码：令牌只能读指标，即使泄露也无法改密码、
无法退出你的登录、也无法给自己发新令牌（管理令牌必须是页面会话身份）。
撤销后立即失效；`auth.json` 里只存哈希，明文只在创建那一刻返回一次。
