...
| `DASHBOARD_API_TOKEN` | 无 | 启动时播种一个只读令牌（**仅在还没有任何令牌时生效**），适合容器/脚本下发 |
| `DASHBOARD_TRUST_PROXY` | 关 | 置 1 时采信 `X-Forwarded-For`（反代后必须开，否则限速会算成代理一个 IP） |
...
**只读 API Token**（Bot / 脚本用，避免保存账号密码）：

- 页面侧栏「账号」→「新建只读令牌」，名称随意（如 `bot`）；完整令牌**只显示一次**；
- 用 `Authorization: Bearer dshk_xxx` 或 `?token=dshk_xxx` 访问，任何写接口一律 `403 read_only`；
- 令牌不能创建/撤销令牌（管理必须在页面上以会话身份做）；撤销即时生效；
- 服务端只存 SHA-256（256 位随机量，无需慢哈希）；最多 20 个，超出淘汰最久未用的；
- 也可以用 `DASHBOARD_API_TOKEN` 在启动时播种一个（仅当还没有任何令牌时生效）。
...
整页信息基本不变，后端缓存 **60 秒**（其中要起 `bluetoothctl` 与一次 `docker --version`）。
...
- **初始密码会打印到日志**（`journalctl -u dashboard | grep auth`）：journald 只有特权用户可读；
  用 `run.sh` 时日志文件是 600（`run.sh` 里设了 `umask 077`）。改完密码后建议清掉那一行。
- **放在反向代理后面时必须设 `DASHBOARD_TRUST_PROXY=1`**：否则登录限速按代理 IP 统计，
  代理后的所有客户端共用一个来源，攻击者连错几次就能把唯一账号一起锁在门外
  （默认不信任该头，是为了防止伪造 `X-Forwarded-For` 绕过限速）。
...
6. 参考图左下角的「关闭总控台」改为「暂停更新」：看板是只读监控，不该提供关机入口；
   不做能被同网段任意触发的关机接口。
...
10. 参考图没有「设备」页，按现有视觉语言设计成「与这台机器连接的设备」：顶部一行本机摘要，
    下面 USB / 蓝牙 / 网络接口 / 局域网设备四张卡。（原先的「传感器」页与性能页重复度高，已删除；
    PCI 清单信息量低，也换成了免凭据的局域网发现。）
...
- 设备页依赖 USB/蓝牙子系统与邻居表：容器里可能读不到宿主机的设备，会如实显示「不可用」或「无适配器」。
- 局域网扫描只覆盖本机物理网卡所在网段；开了 AP 隔离或访客网络的设备扫不到。
