...
- **多页面（分批上线）**：前端为 hash 路由单页应用，页面模块按需加载；
  ...
  与「进程」（全量列表、搜索、点列头锁定排序、点行展开）、「网络与磁盘」
  （网速曲线、连接概况、网卡信息、磁盘容量与读写曲线）、「服务」
  （可配置的远程探测、容器、监听端口与「谁能访问」、systemd 服务），其余页签先显示「建设中」。
...
10. 网络页里参考图的「Wi-Fi 信号 / 附近的 Wi-Fi」两张卡，本机是 USB 有线网卡、没有无线，
