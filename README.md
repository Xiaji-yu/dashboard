...
- **多页面（分批上线）**：前端为 hash 路由单页应用，页面模块按需加载；
  ...
  **七个页面全部上线**：概览、性能与电源（每核占用、温度、风扇、核显频率、内存构成、电池）、
  进程（全量列表、搜索、锁定排序、行展开）、网络与磁盘（网速曲线、连接、网卡、磁盘读写）、
  服务（可配置远程探测、容器、监听端口与「谁能访问」、systemd）、设备（主机/处理器/内存磁盘/环境）、
  传感器（温度通道、风扇转速、每核频率与占用，各带 2 分钟曲线）。
...
| 项 | 口径 |
| --- | --- |
| 主机 | 主机名、`/etc/os-release` 的系统名、内核与架构（`platform`）、运行时长、`/sys/class/dmi/id/*` 的厂商/型号/主板/BIOS（**本机免 root 可读**） |
| 处理器 | `/proc/cpuinfo` 的型号与 flags（`vmx` → VT-x）、物理核/逻辑核、`cpufreq` 的最小/最大频率、逐级缓存（`/sys/devices/system/cpu/cpu0/cache/*`，格式化成 `L1d 32 KB · L2 256 KB · L3 3 MB`） |
| 内存与磁盘 | 内存/交换总量、磁盘型号与容量（`udevadm` + sysfs）、SSD/机械、真实设备分区表 |
| 网络与运行环境 | 网卡链路信息、Python / psutil / Docker 版本、电池（电量/状态/循环次数） |
| 缓存 | 整页信息基本不变，后端缓存 **60 秒**（其中要起一次 `docker --version`） |
| 温度 | 全部 hwmon 温度通道（封装/各核心/PCH/机身），带占用条与 2 分钟双线曲线 |
| 风扇 | 全部风扇转速（`cpu_fan` 为大数字，转速为 0 的如实标「停转」），带 2 分钟曲线 |
| 每核 | 每逻辑核的占用与实时频率（按物理核标出「核心 N · 线程 M」），带 4 线 2 分钟曲线 |

传感器页没有新增接口：直接复用 `/api/performance` 与 `/api/series`。
...
10. 参考图没有「设备」「传感器」两页，按现有视觉语言设计：设备页是四张信息卡（主机 / 处理器 /
    内存与磁盘 / 网络与运行环境），传感器页是温度、风扇、每核三块，各带 2 分钟曲线。
...
- 传感器页与设备页依赖 hwmon / DMI：虚拟机或容器里可能读不到温度或机型信息，会如实显示「不可用」。
